...
| TL-01 | Configurar calendário independente por mundo, com prévia de anos negativos, zero e positivos | implementado e verificado por testes de domínio, migração e filesystem | `src/domain/worlds/calendar.ts`, `/calendar`, `worldMigrations` 15→16 e testes correspondentes | Integração com datas de entidades entregue no TL-02; composição de linhas do tempo permanece futura | 23/09/2026 |
| TL-02 | Editar e ler datas de propriedades no calendário do mundo, mantendo ISO legado legível | implementado e verificado por testes de domínio e filesystem | `src/domain/worlds/calendar-date.ts`, editor de propriedades, leitor de entidades, `calendar-date.test.ts` e round-trip de Evento/Organização | Conversão em lote com prévia, inválidos, backup e confirmação pertence ao TL-03 | 24/09/2026 |
