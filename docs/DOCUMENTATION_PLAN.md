...
- A persistência atual é por arquivos JSON. Não há banco relacional nem migrações SQL. Os esquemas atuais são `World` v15, `Entity` v4, `ContentDocument` v2 e `Asset`, `Board` e `Scene` v1, conforme `src/domain/`.
