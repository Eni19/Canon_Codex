...
- O corpo de texto de uma entidade é persistido separado dos metadados, em `content.json`, como um
  `ContentDocument`: `{ format: 'tiptap-json', schemaVersion, body }`, onde `body` é o JSON do
  Tiptap (`JSONContent`) tratado como **opaco** pelo domínio (tipado como `unknown` fora da camada
  do editor).
...
- `ContentRenderer` (leitura, fora do editor) consome o mesmo `body` para renderizar a página em
...
- Trocar de editor no futuro (ou migrar para um novo formato Tiptap major) é: escrever um novo
  `format` no `ContentDocument`, uma função de migração `tiptap-json@1 -> novo-formato@1`, e um novo
  `ContentRenderer` — não uma reescrita do domínio.
