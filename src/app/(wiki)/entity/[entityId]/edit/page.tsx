...
import { importCosmologySymbolAction, importCoverImageAction } from '@/app/actions/assets'
...
export default async function EditEntityPage(props: PageProps<'/entity/[entityId]/edit'>) {
  ...
  return (
    <PageContainer className="max-w-2xl">
      ...
      {entity.type !== 'tale' && <div className="mt-6">
        <Label>{entity.type === 'cosmology' ? 'Arte principal' : 'Imagem principal'}</Label>
        <div className="mt-1.5">
          <AssetPicker currentAssetId={entity.coverAssetId} importAction={boundImportCover} />
        </div>
      </div>}
      {entity.type === 'cosmology' && <div className="mt-6">
        ...
        <div className="mt-1.5">
          <AssetPicker currentAssetId={typeof entity.properties.symbolAssetId === 'string' ? entity.properties.symbolAssetId : undefined} importAction={boundImportSymbol} contain />
        </div>
      </div>}
      ...
      <form action={boundUpdate} className="mt-6 flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <Input id="title" name="title" required defaultValue={entity.title} />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {entity.type !== 'evidence' && <div className="flex flex-col gap-1.5">
            <Label htmlFor="aliases">
              Apelidos <span className="font-normal text-muted-foreground">(separados por vírgula)</span>
            </Label>
            <Input id="aliases" name="aliases" defaultValue={entity.aliases.join(', ')} />
          </div>}
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="tags">
              Tags <span className="font-normal text-muted-foreground">(separadas por vírgula)</span>
            </Label>
            <Input id="tags" name="tags" defaultValue={entity.tags.join(', ')} />
          </div>
        </div>
        {entity.type !== 'evidence' && <div className="flex flex-col gap-1.5 sm:w-1/2 sm:pr-2">
          <Label htmlFor="status">Status</Label>
          <Input id="status" name="status" defaultValue={entity.status ?? ''} />
        </div>}

        {entityType.properties.length > 0 && (
          <div className="mt-2 border-t border-border pt-4">
            <h2 className="mb-3 text-sm font-medium text-muted-foreground">Propriedades</h2>
            <PropertyList properties={entityType.properties} values={entity.properties} referenceOptionsByKey={referenceOptionsByKey} />
          </div>
        )}

        {entity.type === 'location' && (
          <LocationPointsEditor coverAssetId={entity.coverAssetId} initialPoints={parsedPoints.success ? parsedPoints.data : []} targets={pointTargets} />
        )}
        ...
        {(entityType.layout.includes('portraitHero') || entityType.id === 'location' || entityType.id === 'creature' || entityType.id === 'cosmology') && <CharacterThemePicker value={entity.theme} label={entityType.id === 'location' ? 'Cor do local' : entityType.id === 'creature' ? 'Cor da criatura' : entityType.id === 'cosmology' ? 'Paleta da entidade' : 'Cor do personagem'} />}

        <div className="mt-2 border-t border-border pt-4">
          <Label className="mb-1.5">Conteúdo</Label>
          <ContentEditor content={content} theme={entity.theme} />
        </div>
