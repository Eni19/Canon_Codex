...
export default async function ScenePage({ params, searchParams }: { params: Promise<{ sceneId: string }>; searchParams: Promise<{ mode?: string }> }) {
  ...
  const scenes: SceneNavigationSummary[] = sceneList.map((item) => ({ id: item.id, title: item.title, description: item.description, backgroundAssetId: item.background.assetId, locationTitle: entityMap.get(item.background.locationId)?.title ?? 'Local indisponível' }))
  const initialMode = query.mode === 'presentation' ? 'presentation' : query.mode === 'run' ? 'run' : 'edit'
  return <SceneRunner key={scene.id} scene={scene} scenes={scenes} entities={tokenEntities} pois={pois} locationTitle={location?.title ?? 'Local indisponível'} initialMode={initialMode} />
}
