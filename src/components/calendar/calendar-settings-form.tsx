      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          {calendar.intercalaryRules.map((rule, index) => <div key={rule.id} className="grid grid-cols-1 gap-3 rounded-md border border-border p-3 sm:grid-cols-2 lg:grid-cols-3">
            <div><Label htmlFor={`rule-name-${index}`}>Nome</Label><Input id={`rule-name-${index}`} className="mt-1" value={rule.name} onChange={(event) => updateRule(index, { name: event.target.value })} /></div>
            <div><Label htmlFor={`rule-month-${index}`}>Depois do mês</Label><Input id={`rule-month-${index}`} className="mt-1" type="number" min={1} max={calendar.months.length} value={rule.month} onChange={(event) => updateRule(index, { month: Number(event.target.value) })} /></div>
            <div><Label htmlFor={`rule-every-${index}`}>A cada (anos)</Label><Input id={`rule-every-${index}`} className="mt-1" type="number" min={1} value={rule.everyYears} onChange={(event) => updateRule(index, { everyYears: Number(event.target.value) })} /></div>
            <div><Label htmlFor={`rule-offset-${index}`}>Deslocamento</Label><Input id={`rule-offset-${index}`} className="mt-1" type="number" min={0} value={rule.yearOffset} onChange={(event) => updateRule(index, { yearOffset: Number(event.target.value) })} /></div>
            <div><Label htmlFor={`rule-skip-${index}`}>Salta a cada (opcional)</Label><Input id={`rule-skip-${index}`} className="mt-1" type="number" min={1} value={rule.skipEveryYears ?? ''} onChange={(event) => updateRule(index, { skipEveryYears: event.target.value ? Number(event.target.value) : undefined, includeEveryYears: event.target.value ? rule.includeEveryYears : undefined })} /></div>
            <div><Label htmlFor={`rule-include-${index}`}>Inclui a cada (opcional)</Label><Input id={`rule-include-${index}`} className="mt-1" type="number" min={1} value={rule.includeEveryYears ?? ''} onChange={(event) => updateRule(index, { includeEveryYears: event.target.value ? Number(event.target.value) : undefined })} /></div>
