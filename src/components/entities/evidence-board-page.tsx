...
export function EvidenceBoardPage({ entity, entityType, worldName, content, entityTitleById, allEntities }: EntityPageProps) {
  ...
  return <div className={styles.board}><main className={styles.canvas}>
    ...
    <header className={styles.header}><div><Breadcrumbs items={[{label:worldName,href:'/'},{label:entityType.pluralLabel,href:`/${entityType.id}`},{label:entity.title}]}/><h1 className={styles.titleTape}>{entity.title}</h1></div><Button asChild variant="outline" size="sm"><Link href={`/entity/${entity.id}/edit`}><Pencil className="size-4"/>Editar</Link></Button></header>
